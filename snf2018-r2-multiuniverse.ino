...
const int startLED[] = {0, 5};  //starting LEDs for all universes.
...
{   
    ...
    Serial.println(WiFi.localIP());
    //Serial.println(" Universes= "+ maxUniverses);
    if (WiFi.ready()){
...
void onDmxFrame(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t* data)
{
  ...
  for (int i = 0; i < length / 3; i++)//Every universe starts at Ch0.
  {
    
    //int led = i + (universe - startUniverse) * (previousDataLength / 3);
    int led = i + startLED[universe];
